...
const menuItemApiRequest = {
  list: (params?: { categoryId?: string; limit?: number }) =>
    http.get<MenuItemListResType>('api/v1/menuItems', {
      params: { limit: 100, ...params },
    }),

  getById: (id: string) =>
    http.get<MenuItemResType>(`api/v1/menuItems/${id}`),
  ...
  deleteImage: (menuItemId: string, imageId: string) =>
    http.delete<MenuItemResType>(`api/v1/menuItems/${menuItemId}/images/${imageId}`),
