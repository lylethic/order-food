...
const STATUS_CONFIG: Record<string, { bg: string; text: string; pulse: boolean }> = {
  ...
  Delivered: { bg: 'bg-muted',  text: 'text-muted-foreground',   pulse: false },
  Cancelled: { bg: 'bg-rose-50',    text: 'text-rose-500',    pulse: false },
};

export default function StatusBadge({ status }: { status: string }) {
  const cfg = STATUS_CONFIG[status] ?? { bg: 'bg-muted', text: 'text-muted-foreground', pulse: false };
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold ${cfg.bg} ${cfg.text}`}>
