...
export default function ScanInner() {
  const { t, setTableSession } = useAppContext();
  ...
  useEffect(() => {
    ...
    tableApiRequest
      .verifyToken({ table: tableParam, sid: sidParam })
      .then((res) => {
        if (cancelled) return;
        const data = res.payload.data as any;
        const valid = data?.valid ?? data?.data?.valid ?? false;
  ...
  return (
    <div className='min-h-screen flex items-center justify-center bg-gradient-to-br from-neutral-950 via-neutral-900 to-neutral-800 px-4'>
      <div className='absolute top-6 left-1/2 -translate-x-1/2 flex items-center gap-2 text-amber-400'>
        <UtensilsCrossed size={22} />
        <span className='font-semibold tracking-wide text-sm uppercase'>Restaurant</span>
      </div>
          ...
          >
            ...
            <div>
              ...
              <p className='text-neutral-400 mt-1'>
                {t.qrTableLabel}{' '}
                <span className='text-amber-400 font-bold text-xl'>{tableParam}</span>
              </p>
            </div>
